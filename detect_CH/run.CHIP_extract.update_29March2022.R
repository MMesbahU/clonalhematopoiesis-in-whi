...
# Rscript runScript.R outdir driver_gennes annot_txt suffix_2_remove prefix_2_remove
commandLineArgs <- commandArgs(TRUE)

outDir <- commandLineArgs[1]

diver_genes <- commandLineArgs[2]

inAnnotFile <- commandLineArgs[3]

prefix_2_remove <- commandLineArgs[4]

suffix_2_remove <- commandLineArgs[5]
...
load(diver_genes)
##################
  # annot.1000291_23153_0_0-filtered.hg38_multianno.txt.gz 
  sample_id <- gsub(x=gsub(x = basename(inAnnotFile), 
                           pattern = prefix_2_remove,replacement = ""), 
                    pattern = suffix_2_remove, 
                    replacement = "")
  
  cat( paste0("Reading SNPs and INDEL from Sample ", sample_id,"\n"))
  
  vars <- fread(inAnnotFile, 
                header = T, 
                sep="\t")
...
# varsOI<-vars[Gene.refGene%in%gList$Gene,1:10]
  varsOI <- vars[Gene.refGene %in% gList$Gene, ]
 
#
  varsOI.func <- varsOI[grepl("exonic", varsOI$Func.refGene, fixed=T) |
...
## 
	extractTranscript <- function(GeneDetail, AAChange, Accession)
	  ...
	  if(length(splice) >0){return(splice)} 
	  else{
	    if(length(nonsyn) >0){return(nonsyn)} 
	    else{ return("nan") }
	    }
	  }
...
#Apply function
	...
	extractNonsyn <- function(transcriptOI)
	  ...
	  if(length(protChange)>0)
	  {
	    return(gsub("p\\.","",protChange))
	    } else {return("nan")}
	  }
	...
	varsOI.func <- transform(varsOI.func,whitelist=F, 
	                         wl.mis=F,wl.lof=F,wl.splice=F,
	                         wl.exception=F,manualreview=F)
  ...
  asxl1ExceptionSplice <- (varsOI.func$Gene.refGene=="ASXL1") & vSplice & vSpliceCorrectTranscript & (vexon11splice|vexon12splice)
  ...
  #Splice
  asxl2ExceptionSplice <- (varsOI.func$Gene.refGene=="ASXL2") & vSplice & vSpliceCorrectTranscript & (vexon11splice|vexon12splice)
  ...
  TETidx <- which(varsOI.func$Gene.refGene=="TET2" & varsOI.func$ExonicFunc.refGene=="nonsynonymous SNV" & nchar(varsOI.func$NonsynOI)==6)
  ...
  CBLidx <- which(varsOI.func$Gene.refGene=="CBL" & varsOI.func$ExonicFunc.refGene=="nonsynonymous SNV" & nchar(varsOI.func$NonsynOI)==5)
  ...
  CBLBidx <- which(varsOI.func$Gene.refGene=="CBLB" & varsOI.func$ExonicFunc.refGene=="nonsynonymous SNV" & nchar(varsOI.func$NonsynOI)==5)
  ...
  #CREBBP	S1680del
  varsOI.func[varsOI.func$ExonicFunc.refGene=="nonframeshift deletion" & (varsOI.func$Gene.refGene=="CREBBP"),"manualreview"]=T
  ...
  varsOI.func[varsOI.func$ExonicFunc.refGene=="nonframeshift deletion" & (varsOI.func$Gene.refGene=="DNMT3A"),"manualreview"]=T
  
  #EP300	VF1148_1149del
  varsOI.func[varsOI.func$ExonicFunc.refGene=="nonframeshift deletion" & (varsOI.func$Gene.refGene=="EP300"),"manualreview"]=T

  #FLT3	FY590-591GD,	del835
  varsOI.func[(varsOI.func$ExonicFunc.refGene=="nonframeshift deletion" | varsOI.func$ExonicFunc.refGene=="nonframeshift insertion") & (varsOI.func$Gene.refGene=="FLT3"),"manualreview"]=T
  ...
  varsOI.func[(varsOI.func$ExonicFunc.refGene=="nonframeshift deletion" | varsOI.func$ExonicFunc.refGene=="nonframeshift insertion") & (varsOI.func$Gene.refGene=="JAK2"),"manualreview"]=T
  
  #KDM6A	del419
  varsOI.func[(varsOI.func$ExonicFunc.refGene=="nonframeshift deletion") & (varsOI.func$Gene.refGene=="KDM6A"),"manualreview"]=T
  
  #KIT	ins503,	del560,	del579,	del551-559
  varsOI.func[(varsOI.func$ExonicFunc.refGene=="nonframeshift deletion" | varsOI.func$ExonicFunc.refGene=="nonframeshift insertion") & (varsOI.func$Gene.refGene=="KIT"),"manualreview"]=T

  #MPL	del513 W515-518KT
  varsOI.func[(varsOI.func$ExonicFunc.refGene=="nonframeshift deletion" | varsOI.func$ExonicFunc.refGene=="nonframeshift insertion") & (varsOI.func$Gene.refGene=="MPL"),"manualreview"]=T

  #NPM1	Frameshift p.W288fs (insertion at c.859_860, 860_861, 862_863, 863_864)
  varsOI.func[(varsOI.func$ExonicFunc.refGene=="frameshift deletion" | varsOI.func$ExonicFunc.refGene=="frameshift insertion") & (varsOI.func$Gene.refGene=="NPM1"),"manualreview"]=T

  #SRSF2	P95fs
  varsOI.func[vFS & (varsOI.func$Gene.refGene=="SRSF2"),"manualreview"]=T
  
#write out file
  
  fwrite(varsOI.func, paste(sample_id, ".annovar.varsOI.allvariants.csv.gz", sep=""), row.names=FALSE, sep=",")

  fwrite(varsOI.func[whitelist==T,], paste0(sample_id,".varsOI.wl.csv.gz"), row.names = F, sep=",")
  
  fwrite(varsOI.func[manualreview==T,],paste0(sample_id,".varsOI.manualreview.csv.gz"), row.names = F, sep=",")

  fwrite(varsOI.func[Accession=="NM_022552" & ExonicFunc.refGene=="nonsynonymous SNV" &!whitelist,], paste0(sample_id,".varsOI.DNMT3Amis.notwl.csv.gz"), row.names = F, sep=",")
